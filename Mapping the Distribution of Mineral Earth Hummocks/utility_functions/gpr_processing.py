...
def calculate_frost_table_depths_and_bulk_volumetric_soil_water_content(gdf: gpd.GeoDataFrame, savgol_window_length: int = 251, savgol_poly_order: int = 3) -> gpd.GeoDataFrame:
    ...
    filtered_values = savgol_filter(data["volumetric_soil_water_content"].fillna(0), savgol_window_length, savgol_poly_order)
    data.loc[:, "volumetric_soil_water_content"] = pd.Series(filtered_values, index=data.index)
